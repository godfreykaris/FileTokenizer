...
json Server::process_post_request(const json& request_json)
{
        ...
        request_json.find("file_extensions") != request_json.end())
    {
        ...
        std::string extensions_string = std::accumulate(
                extensions.begin() + 1, // Start from the second element
                extensions.end(),
                extensions.front(),     // Initial value is the first element
        ...
        {        
            ...
            std::cout<<"\n\nTotal found files with extensions:\n"<< extensions_string << "\nAre: "<<file_paths.size()<< "\n" <<std::endl;

            json processing_complete_response;
            processing_complete_response["start"] = true;
            std::string url = ai_microservice_url_ + "/start_processing";
            bool success = send_post_request(url, processing_complete_response);
            if (!success)
            {
            ...
            if(file_paths.size() > 0)
            {
                ...
                {
                    thread_pool_.enqueue(
                    [this, file_path = file_path, directory_iterator, file_paths]
                    {
                        ...
                        if(created_tokens.size() > 0)
                        {
                            ...
                            processing_complete_response["file_path"] = path.string();

                            processed_files += 1;

                            std::cout<<"\nProcessed files: "<< processed_files << " Total files:" << file_paths.size()<<std::endl;

                            std::string url = ai_microservice_url_ + "/process_file";
                            bool success = send_post_request(url, processing_complete_response);
                        ...
                        if (file_path == file_paths[file_paths.size() -1])
                        {
                            std::cout<<"\nProcessed files: "<< processed_files<<std::endl;
                            json processing_complete_response;

                            processing_complete_response["total_files"] = processed_files;

                            std::string url = ai_microservice_url_ + "/report_task_complete";
            ...
            {
                std::cout<<"Processed files: "<< processed_files<<std::endl;
                json processing_complete_response;
...
bool Server::send_post_request(const std::string& target_url, const json& post_data) {
    ...
    for (int attempt = 1; attempt <= max_attempts; ++attempt) {
        // Set the target URL
        curl_easy_setopt(curl_.get(), CURLOPT_URL, target_url.c_str());
        // Set the request data
        const std::string json_data = post_data.dump();
        curl_easy_setopt(curl_.get(), CURLOPT_POSTFIELDS, json_data.c_str());
